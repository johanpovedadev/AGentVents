...
async function parseIntent(message) {
  ...
  const schema = '{"tipo":"crear_contacto|crear_deal|actualizar_deal|actualizar_contacto|generar_informe","datos":{}}';
  const prompt = `Convierte esta instrucción CRM al siguiente JSON exacto: ${schema}. Responde SOLO JSON válido, sin Markdown ni explicación. Para crear_deal usa contactId si está disponible; para actualizar_deal usa dealId y dealstage; para actualizar_contacto usa contactId y propiedades. Instrucción: ${message}`;

  try {
    const gemini = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
    const model = gemini.getGenerativeModel({ model: 'gemini-2.0-flash', generationConfig: { responseMimeType: 'application/json' } });
    const result = await model.generateContent(prompt);
    const text = result.response.text().trim().replace(/^```json\s*|\s*```$/g, '');
    const intencion = JSON.parse(text);
    const validTypes = ['crear_contacto', 'crear_deal', 'actualizar_deal', 'actualizar_contacto', 'generar_informe'];
